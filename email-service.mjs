...
export async function sendTestDriveConfirmationEmail({
  ...
  const smtpUser = cleanStr(process.env.SMTP_USER);
  const smtpPass = cleanStr(process.env.SMTP_PASS);
  const smtpHost = cleanStr(process.env.SMTP_HOST) || 'smtp.gmail.com';
  const smtpPort = Number(cleanStr(process.env.SMTP_PORT) || 587);
  const smtpSecure = process.env.SMTP_SECURE === 'true' || smtpPort === 465;
  const smtpFrom = cleanStr(process.env.SMTP_FROM) || `EasyEV <${smtpUser}>`;
